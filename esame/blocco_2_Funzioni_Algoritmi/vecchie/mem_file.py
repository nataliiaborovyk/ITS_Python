...
def memoriz(lista:list[int]):
    ...
    for file in lista:
        ...
        file_zip = file * 0,8
        dim_zip += file_zip
        bloc_nec = int(file_zip/512)
        bloc_us += bloc_nec
        bloc_rim = blocchi - bloc_us
        if bloc_rim >= 0:
            print(f"File di {file} byte compresso in {file_zip} byte e memorizzato. Blocchi usati: {bloc_nec}. Blocchi rimanenti: {bloc_rim}")
        else:
